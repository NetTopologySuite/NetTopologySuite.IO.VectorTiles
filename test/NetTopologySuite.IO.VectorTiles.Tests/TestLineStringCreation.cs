...
{
    public class TestLineStringCreation
    {
        IGeometry suspect =
        ...
        [Fact]
        public void GenerateTiles_SuspectLinstringZoom21_Works()
        {
            var features = new FeatureCollection();
            features.Add(new Feature(suspect, new AttributesTable()));

            IEnumerable<(IFeature feature, int zoom, string layerName)> ConfigureFeature(IFeature feature)
            {
                yield return (feature, 21, "layername");
            }

            new VectorTileTree {{features, ConfigureFeature}}.Write("testdir");
        }
        [Fact]
        public void GenerateTiles_SuspectLinstringZoom22_Works()
        {
            var features = new FeatureCollection();
            features.Add(new Feature(suspect, new AttributesTable()));
            ...
            IEnumerable<(IFeature feature, int zoom, string layerName)> ConfigureFeature(IFeature feature)
            {
                yield return (feature, 22, "layername");
            }
